...
namespace Microsoft.CodeAnalysis.Sarif.PatternMatcher.Cli
{
    ...
    internal class StressCommand : CommandBase
    {
        ...
        private void RunMultiThreadedTelemetry(StressOptions options)
        {
            ...
            sb.AppendLine($"{DateTime.Now}, {timer.ElapsedMilliseconds}, {cpuUtilization}, {(double)Process.GetCurrentProcess().PeakWorkingSet64 / 1000000}");
        ...
        {
            var perFileSb = new StringBuilder($"Filename, File Size in KB, Runtime in ms, %-CPU Utilization, Peak Mem (MB){Environment.NewLine}");
...
;
        ...
        private void TimeScanFileWithSkimmers(string filePath, ISet<Skimmer<AnalyzeContext>> skimmers)
        {
            ...
            double cpuUtilization = 100 * (endCpu.TotalMilliseconds - startCpu.TotalMilliseconds) / (Environment.ProcessorCount * timer.ElapsedMilliseconds);
            fileDataTupleList.Add(Tuple.Create(filePath.Replace(',', ';'), fileSystem.FileInfoLength(filePath) / 1024, timer.ElapsedMilliseconds, cpuUtilization, (double)Process.GetCurrentProcess().PeakWorkingSet64 / 1000000));
        }
