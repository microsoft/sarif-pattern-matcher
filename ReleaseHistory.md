...
- BUG: Properly report command-line help rather than raising `IndexOutOfRangeException` when invoking tool with no arguments.
- NEW: All secret detecting rules now provide a `sarif/uiLabel` value in the property bag that represents a human-readable UX label for the check.
